...
```{r filter}
reads_threshold <- 4
molecules <- molecules_orig %>% filter(Reads >= reads_threshold) %>%
...
molecules_large_scaffolds <- molecules %>% filter(as.integer(Rname) <= rname_threshold)

molecules_good <- molecules_large_scaffolds %>%
	filter(AS_median > as_median_threshold & Size >= size_threshold)

molecules_large_scaffolds_good <- molecules_good %>% filter(as.integer(Rname) <= rname_threshold)
```
...
```{r read-density}
# Molecules that span the scaffold4 misassembly.
scaffold4_spanners <- molecules %>% filter(Rname == "4" & Start < 108910 & End > 109284)
...
ggplot(molecules) +
	...
	geom_vline(data = scaffold4_spanners, aes(xintercept = LogReadDensity), alpha = 0.5)
...
molecules_starts <- molecules_large_scaffolds %>%
	filter(AS_median >= starts_as_threshold & Size >= starts_size_threshold) %>%
	group_by(Rname, Start) %>%
...
ggplot(molecules_starts) +
	...
	geom_point(aes(x = Pos, y = Depth, colour = Depth < depth_good_threshold)) +
	geom_point(aes(y = Starts)) +
	geom_vline(aes(xintercept = Pos), misassemblies) +
...
ggplot(molecules_large_scaffolds_good) +
