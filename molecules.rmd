...
```{r misassemblies}
molecules_large_scaffolds <- molecules %>% filter(as.integer(Rname) <= 20)
...
molecules_starts <- molecules_large_scaffolds %>%
	filter(Size >= starts_size_threshold) %>%
	group_by(Rname, Start) %>%
	summarize(Starts = n(), Starts_AS_median = median(AS_median), Starts_sum_reads = sum(Reads)) %>%
	ungroup() %>%
	rename(Pos = Start) %>%
...
```{r misassemblies_tsv}
write_tsv(misassemblies, "misassemblies.tsv")
```
