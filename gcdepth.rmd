...
composition_depth <- composition %>%
  ...
  replace_na(list(Organelle = "Unknown"))
...
plot_length_depth_point <- ggplot(composition_depth %>% filter(NumACGT > 500)) +
  aes(x = Depth, y = NumACGT, colour = Organelle, size = NumACGT) +
...
plot_length_depth_point +
  ...
  scale_y_log10(breaks = c(1e3, 1e4, 1e5, 1e6))
plot_length_depth_point
plot_length_depth_point + coord_cartesian(c(0, 100), c(0, 20000))
```
...
plot_gc_content_depth + coord_cartesian(c(0, 100))
```
...
ggplot(composition_depth_large %>% filter(Depth < 100)) +
  aes(x = Depth, y = GC, fill = Organelle, size = NumACGT, weight = NumACGT) +
  scale_fill_manual(values = c(Mitochondrion = "blue", Plastid = "green", Unknown = "black")) +
  geom_bin2d()
```
